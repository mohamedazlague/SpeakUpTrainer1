...
if (-not (Test-Path $JdkDir)) {
    ...
    if (-not (Test-Path $JdkZip)) { curl.exe -L -o "$JdkZip" "$JdkUrl" }
...
if (-not (Test-Path $CmdLineToolsLatest)) {
    ...
    if (-not (Test-Path $AndroidSdkZip)) { curl.exe -L -o "$AndroidSdkZip" "$AndroidSdkUrl" }
...
if (-not (Test-Path $GradleDir)) {
    ...
    if (-not (Test-Path $GradleZip)) { curl.exe -L -o "$GradleZip" "$GradleUrl" }
